...
/// Dual-path media picker.
///
/// * **Android 13+ (API 33+):** launches the system **Photo Picker** via
///   `image_picker.pickMultipleMedia()`. No media permissions declared, no
///   prompts, Play Store Photo & Video Permissions policy satisfied.
/// * **Android ≤12 and iOS:** falls back to an in-app gallery grid powered
///   by `photo_manager`, because the Photo Picker backport isn't available
///   on many older / OEM-customised Android builds (e.g. OnePlus
///   OxygenOS 11), where the generic `ACTION_GET_CONTENT` fallback opens a
///   single-select file browser. The custom grid only needs
///   `READ_EXTERNAL_STORAGE` (maxSdkVersion=32) — **not** subject to the
...
class _PickerScreenState extends ConsumerState<PickerScreen>
  ...
  // Null until we detect the platform; true → Photo Picker path, false →
  // custom gallery path.
  bool? _useSystemPhotoPicker;

  // Custom-gallery state (only used when _useSystemPhotoPicker == false).
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (_useSystemPhotoPicker == false &&
  ...
  Future<void> _decidePath() async {
    final useSystem = await _shouldUseSystemPhotoPicker();
    if (!mounted) return;
    setState(() => _useSystemPhotoPicker = useSystem);
    if (useSystem) {
      WidgetsBinding.instance.addPostFrameCallback((_) => _launchSystemPicker());
    } else {
      _checkAndRequest();
    }
  }

  Future<bool> _shouldUseSystemPhotoPicker() async {
  ...
  // ── Android 13+ / iOS path: system Photo Picker ───────────────────────────
  Future<void> _launchSystemPicker() async {
    try {
      ...
      // Fall through to the custom gallery so user can still complete the flow.
      if (mounted) setState(() => _useSystemPhotoPicker = false);
      _checkAndRequest();
    }
  }

  // ── Android ≤12 path: custom gallery via photo_manager ────────────────────
  ...
  Widget build(BuildContext context) {
    if (_useSystemPhotoPicker == null || _useSystemPhotoPicker == true) {
      return const Scaffold(
        backgroundColor: AppColors.bgDark,
        body: Center(child: CircularProgressIndicator()),
      );
    }
    return _buildCustomGalleryScaffold();
  }

  Widget _buildCustomGalleryScaffold() {
    return Scaffold(
      backgroundColor: AppColors.bgDark,
      appBar: AppBar(
        title: Column(
